...
-- FIX 5: Enhanced cleanup — disconnects, Drawing objects, ScreenGuis, and state reset.
-- Previously only disconnected connections, leaving orphaned Drawing objects and GUIs.
local _drawingCleanup = {} -- populated as Drawing objects are created
...
local function cleanupAll()
    ...
    -- Remove all tracked Drawing objects
    for _, d in ipairs(_drawingCleanup) do
        _safeCall(function() d:Remove() end)
    end
    _drawingCleanup = {}

    -- Destroy Amethyst ScreenGuis
    _safeCall(function()
        for _, gui in ipairs(CoreGui:GetChildren()) do
            if gui.Name == "AmethystWatermarkV3" or gui.Name == "AmethystToggleV3"
                or gui.Name == "AmethystFlyButtons" then
                gui:Destroy()
            end
        end
...
local AmethystTheme = {
    ...
    -- FIX 3: Added missing fields required by Rayfield custom theme spec
    TabTextColor                  = THEME.TextPrimary,
    SelectedTabTextColor          = Color3.fromRGB(255, 255, 255),
    SecondaryElementStroke        = Color3.fromRGB(60, 30, 85),
}

-- ============================================================
-- WINDOW (pcall-wrapped with fallback)
-- ============================================================
-- FIX 3: Pass AmethystTheme table directly as Theme value (Rayfield custom theme spec).
local Window
...
for i = 1, 4 do
    ...
    xhLines[i] = l
    _drawingCleanup[#_drawingCleanup + 1] = l
end
...
_drawingCleanup[#_drawingCleanup + 1] = fovCircle
...
_drawingCleanup[#_drawingCleanup + 1] = alertCircle
...
_drawingCleanup[#_drawingCleanup + 1] = alertText
...
_drawingCleanup[#_drawingCleanup + 1] = radarBg
...
_drawingCleanup[#_drawingCleanup + 1] = radarBorder
...
_drawingCleanup[#_drawingCleanup + 1] = radarSelf
...
_drawingCleanup[#_drawingCleanup + 1] = radarLabel
...
for _, d in ipairs({hudPBg, hudBBg, hudPTxt, hudBTxt, killBg, killTxt}) do
    _drawingCleanup[#_drawingCleanup + 1] = d
end
    ...
    _safeCall(function()
        ...
        flyConn = RunService.RenderStepped:Connect(function()
            ...
            _safeCall(function()
                ...
                -- FIX 4: MoveDirection is already a world-space unit vector that accounts
                -- for camera rotation. Decomposing it with camCF axes + .X/.Z treated as
                -- local inputs produced incorrect directions when the camera was rotated.
...
local function onCharAdded(player, char)
    ...
    -- FIX 10: AncestryChanged fires for ANY ancestry change, including removal.
    ...
    -- FIX 6: createESP is now synchronous here (was task.defer), so ESPPool[player]
    -- is guaranteed to exist when accessed on the next line.
...
track(Players.PlayerRemoving:Connect(function(p)
    ...
    -- FIX 9: Only prune weapon cache entries belonging to the leaving player's tools,
    -- not the entire cache. Nuking all entries forced a full rescan of the local
    -- player's equipped tool on the very next frame for no reason.
    if p.Character then
...
_safeCall(function()
    ...
    -- FIX: Removed duplicate UIGradient (toggleGrad) — only one UIGradient allowed per GuiObject.
    -- The accent gradient below (lines ~2768-2774) is the intended one.
    toggleBtn.Text = "A"
    ...
    -- FIX 11: Track this persistent UIS connection for cleanup (FIX 15 compliance)
    track(UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            -- FIX: Use max displacement instead of accumulating (delta is from dragStart, already cumulative)
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        if totalDragDist > 10 then totalDragDist = 0; return end -- was a drag, not click (FIX: reset for next tap)
...
-- Re-hook on respawn (FIX 11: track for cleanup)
        ...
        if target then
            ...
            if pc then
                ...
                if aim then
                    ...
                    -- FIX 7: Delta-time corrected lerp — previously frame-rate dependent
                    -- (faster at 60fps than 30fps). Now uses exponential decay for
                    -- consistent smoothing regardless of framerate.
                    local aimAlpha = math.clamp(1 - math.exp(-10 / S.AimbotSmooth * delta), 0, 1)
                    Camera.CFrame = Camera.CFrame:Lerp(
...
-- (Anti-AFK jump timer merged into consolidated Heartbeat above)

-- (Auto Hop timer merged into consolidated Heartbeat above)
